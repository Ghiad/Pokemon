...
int main()
{
	...
	const char *sqlSentence = "CREATE TABLE IF NOT EXISTS consumer(id integer PRIMARY KEY AUTOINCREMENT,owner text NOT NULL,race text NOT NULL,name text NOT NULL,level integer,exp integer)";
	...
	while (1)
	{
		...
		while (1) {
			...
			if (rec == "注册") {
				...
				memset(sendBuf, 0, MAX_LENGTH);
				memset(recvBuf, 0, MAX_LENGTH);
				sprintf_s(sendBuf, "请输入你所要登陆用户的用户名\n");
				send(connSocket, sendBuf, strlen(sendBuf) + 1, 0);
				recv(connSocket, recvBuf, MAX_LENGTH, 0);
				rec = recvBuf;
				...
				while (!success) {//检查是否有相同用户名
					...
					if (result == SQLITE_OK) {
						...
						if (a == rec) {
							sprintf_s(sendBuf, "登陆成功！\n");
							send(connSocket, sendBuf, strlen(sendBuf) + 1, 0);
							loginuser.push_back(rec);
							success = true;
						}
						else {
							sprintf_s(sendBuf, "登陆失败，请重新输入\n");
							send(connSocket, sendBuf, strlen(sendBuf) + 1, 0);
							recv(connSocket, recvBuf, MAX_LENGTH, 0);
							rec = recvBuf;
						}		
